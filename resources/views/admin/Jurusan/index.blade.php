    ...
    <div class="container-fluid ">
        <div class="card">
            <div class="card-header bg-primary">
                <h4 class="text-white">Data Jurusan</h4>
            </div>
            @if (session('success'))
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="collapse"
                                            data-bs-target="#editJurusanForm-{{ $jurusan->id }}"
                                            onclick="closeOtherEditForms('{{ $jurusan->id }}')">
                                            Edit
                                        </button>
        ...
        <div class="collapse mt-3 mb-4" id="addJurusanForm">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="jurusan" name="jurusan"
                                placeholder="Teknik Kendaraan Ringan" required>
                        </div>
                    </div>
                    <button type="submit" class="btn btn-primary">Simpan</button>
                    <button type="button" class="btn btn-secondary" data-bs-toggle="collapse"
                        data-bs-target="#addjurusanForm">Batal</button>
                </form>
            </div>
        </div>

        @foreach ($jurusans as $jurusan)
            <div class="collapse mt-3 mb-4 edit-form-container" id="editJurusanForm-{{ $jurusan->id }}">
                <div class="card card-body">
                    ...
                    <form action="{{ route('admin.jurusan.update', $jurusan->id) }}" method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="edit_jurusan-{{ $jurusan->id }}" class="form-label">Nama Jurusan</label>
                                <input type="text" class="form-control" id="edit_jurusan-{{ $jurusan->id }}"
                                    name="jurusan" value="{{ $jurusan->jurusan }}" required>
                            </div>
                        </div>
                        <button type="submit" class="btn btn-primary">Update</button>
                        <button type="button" class="btn btn-secondary" data-bs-toggle="collapse"
                            data-bs-target="#editJurusanForm-{{ $jurusan->id }}">Batal</button>
                    </form>
                </div>
            </div>
        @endforeach
...
@push('scripts')
    <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                @if (isset($editMode) && $editMode)
                    var editForm = new bootstrap.Collapse(document.getElementById(
                        'editJurusanForm-{{ $Jurusan->id ?? '' }}'));
                    editForm.show();
                @else
                    var addForm = new bootstrap.Collapse(document.getElementById('addJurusanForm'));
                    addForm.show();
                @endif
            });
        @endif
        ...
        function closeOtherEditForms(currentId) {
            ...
            const addForm = new bootstrap.Collapse(document.getElementById('addJurusanForm'), {
                toggle: false
            });
            addForm.hide();
        }
